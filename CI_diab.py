...
from misc import transformMatrix, psi, calculateDensity,\
    densityAtSinglePoint, printMatrix, V11, V11_2, V22


def mainDiabatic(max_m, max_n, zeros, b, L, c, delta, reducedBasis,
                 representation, stateToCompute):
    """
    ...
    T = calcKinEnDiabaticRep(max_m, max_n, b, zeros, dim)
    print("Calculating potential energy:\n")
    # Buidling potential energy matrix elements
    V, transformationMatrix, coeff, cs = calcPotEnDiabaticRep(max_m, max_n,
                                                              b, zeros, dim,
                                                              L, c, delta)
...
#    Density at the intersection point
    ...
    if (reducedBasis == 1):
        expansionCoefficientsTransformed = np.dot(transformationMatrix,
                                                  eigenvecs)
        totalDensity = densityAtSinglePoint(
            rCoordinate, phiCoordinate, b, dim, zeros,
            expansionCoefficientsTransformed, coeff, transformationMatrix,
            representation, stateToCompute)

        calculateDensity(
            b, dim, zeros, expansionCoefficientsTransformed, coeff,
            transformationMatrix, representation, stateToCompute)
...
#         np.set_printoptions(precision=2)
        ...
        for i in range(2*dim):
            coeff1[i] = str(
                round(np.abs(
                expansionCoefficientsTransformed[i, stateToCompute]), 3))\
                + "(" + str(coeff[i][0]) + "," +  str(coeff[i][1]) + ")"+cs[i]
        coeff1 = coeff1.decode("utf-8")
        print("\nThe following m, n contribute to the wave function:")
        for j in range(1):
            significantIndices\
                = np.abs(expansionCoefficientsTransformed[:, j]) > 0.00001
        ...
        totalDensity = densityAtSinglePoint(
            rCoordinate, phiCoordinate, b, dim, zeros, eigenvecs, coeff,
            transformationMatrix, representation, stateToCompute)
        calculateDensity(b, dim, zeros, eigenvecs, coeff, transformationMatrix,
                         representation, stateToCompute)
...
#         np.set_printoptions(precision=2)
        ...
        for i in range(2*dim):
            coeff1[i] = str(
                round(np.abs(eigenvecs[i, stateToCompute]), 3))\
                + "(" + str(coeff[i][0]) + "," + str(coeff[i][1])\
                + ")" + cs[i]
        coeff1 = coeff1.decode("utf-8")
        print("\nThe following m, n contribute to the wave function:")
        for j in range(1):
    ...
    print("\n ----- This calculation time is {0:.2f} s----- ".
          format(last-ini))
    return(E)
...
def calcPotEnDiabaticRep(max_m, max_n, b, zeros, dim, L, c, delta):
    ...
    def A(n1, n2, b, zeros):
        """Tranformation matrix getting rid of the basis
        function that is 0 at CI"""

        result = 2 / (np.square(b) * sp.jv(1, zeros[0, n1-1])
                      * sp.jv(1, zeros[0, n2-1]))
        return(result)

    V = np.zeros((2*dim, 2*dim), dtype=np.complex128)
    VT = np.zeros((2*dim, 2*dim))

    # Low-level callback routine which uses c library to speed up
    # integration using quad
    # V11: psi(r, m1, n1) * psi(r, m2, n2) * r * (L^2/4 + r**2) / 2
    # (substitute for V11)
    # V11_offDiag: psi(r, m1, n1) * psi(r, m2, n2) * L * r**2 / 2
    # (substitute for V11_2)
    # V22: psi(r, m1, n1) * psi(r, m2, n2) * r * (L**2 + 4*r**2 - 8*delta )
    # / 4 / 2 )

    V11_cfunc = ctypes.CDLL(os.path.abspath('V11.so'))
    V11_cfunc.f.restype = ctypes.c_double
    V11_cfunc.f.argtypes = (ctypes.c_int, ctypes.POINTER(ctypes.c_double),
                            ctypes.c_void_p)

    V11_2_cfunc = ctypes.CDLL(os.path.abspath('V11_2.so'))
    V11_2_cfunc.f.restype = ctypes.c_double
    V11_2_cfunc.f.argtypes = (ctypes.c_int, ctypes.POINTER(ctypes.c_double),
                              ctypes.c_void_p)

    V22_cfunc = ctypes.CDLL(os.path.abspath('V22.so'))
    V22_cfunc.f.restype = ctypes.c_double
    V22_cfunc.f.argtypes = (ctypes.c_int, ctypes.POINTER(ctypes.c_double),
                            ctypes.c_void_p)

    # array to pass to the c library
    # (m1, n1, m2, n2, b, L, delta)
    mn1 = ctypes.c_double * 7
    kk = 0

    # this array allows to convert k to m,n
    coeff = [[0] * dim for i in range(2 * dim)]
        ...
        for m2 in range(-max_m, 1):
            for n1 in range(1, (max_n+1)):
                for n2 in range(n1, (max_n+1)):
                    ...
                    # m1 = m2, using V11 integral (see Mathematica notebook)
                    if m1 == m2:
                        if V[k1-1, k2-1] == 0.0:
                            func = LowLevelCallable(V11_cfunc.f, user_data)
                            V[k1-1, k2-1] = quad(
                                func, 0, b, epsrel=0, epsabs=1e-12,
                                limit=10000, maxp1=500, limlst=500)[0]
#                             V[k1-1, k2-1], error = fixed_quad (V11, 0, b,
#                             args=(m1, n1, m2, n2, L, b, zeros), n = 1000)
                            kk += 1
                            ...
                            if m1 == 0 and m2 == 0:
                                # Constructing matrix to throw away the
                                # bf which is 0 at CI
...
#                             V[k1-1, k2-1], error = fixed_quad (V11_2, 0, b,
#                             args=(m1, n1, m2, n2, L, b, zeros), n = 1000)
                            ...
                            # V12= 2 * c / L i * V11_2, so we
                            # don't need to calculate it
                            V[k1-1, k2-1+dim] = 2 * 1j * c / L * V[k1-1, k2-1]\
                                * (m2-m1)
    ...
    # The following section has a transformation of basis to get rid of
    # the function that is zero at the intersection. This will allow to show
    # that the density at the intersection can be nonzero, and that it lowers
...
def calcKinEnDiabaticRep(max_m, max_n, b, zeros, dim):
    """
    Kinetic energy matrix elements are simply solutions of the Schrodinger
    equation for a free particle in the cylindrical potential T=jmn^2 / 2b^2,
...
if __name__ == "__calcKinEnDiabaticRep__":
    calcKinEnDiabaticRep()
if __name__ == "__calcPotEnDiabaticRep__":
    calcPotEnDiabaticRep()
if __name__ == "__mainDiabatic__":
    mainDiabatic()
if __name__ == "__calculateProbDensityAtSinglePoint__":
    calcPotEnDiabaticRep()
