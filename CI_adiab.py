...
from misc import transformMatrix, isHermitian, psi, calculateDensity,\
    densityAtSinglePoint, printMatrix, largeBasFuncComp
        ...
        for m2 in range(-max_m, 1):
            for n1 in range(1, (max_n+1)):
                for n2 in range(1, (max_n+1)):
                    ...
                    coeff[k1-1+2*dim][0], coeff[k1-1+3*dim][0] = m1, m1, m1, m1
                    coeff[k1-1][1], coeff[k1-1+dim][1],\
                    coeff[k1-1+2*dim][1], coeff[k1-1+3*dim][1] = n1, n1, n1, n1
                    coeff[k11-1][0], coeff[k11-1+dim][0], coeff[k11-1+2*dim][0],\
                    coeff[k11-1+3*dim][0] = -m1, -m1, -m1, -m1
                    coeff[k11-1][1], coeff[k11-1+dim][1], coeff[k11-1+2*dim][1],\
                    coeff[k11-1+3*dim][1] = n1, n1, n1, n1
                    
                    cs[k1-1], cs[k1-1+dim],\
                    cs[k1-1+2*dim], cs[k1-1+3*dim] = 'Ic', 'Is', 'Jc', 'Js'
                    cs[k11-1], cs[k11-1+dim],\
                    cs[k11-1+2*dim], cs[k11-1+3*dim] = 'Ic', 'Is', 'Jc', 'Js'
