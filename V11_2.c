...
/* gcc -L/usr/local/lib -I/usr/local/include -lgsl -lgslcblas -shared -o V11_2.so V11_2.c */
...
double f(int m, double *x, void *user_data) {
	...
	* gsl_sf_bessel_Jn(c[2],gsl_sf_bessel_zero_Jnu(c[2],c[3])*x[0]/c[4])\
	...
	/ (c[4]*gsl_sf_bessel_Jn(c[0]+1,gsl_sf_bessel_zero_Jnu(c[0],c[1])))*c[5]/2*x[0]*x[0];
}
