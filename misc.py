...
def densityAtSinglePoint(r, phi, b, dim, zeros, eigenvecs_T2, coeff,
                         transformationAB, representation, nState):
    ...
    if (representation == "diabatic"):
        for k in range(0, dim):
            ...
            psi2 += psi(r, phi, m, n, zeros, b) * (eigenvecs_T2[k + dim,
                                                                nState])
        totalDensity = np.dot(psi1, np.conjugate(psi1))\
            + np.dot(psi2, np.conjugate(psi2))
    else:
        for k in range(0, dim):
            m, n = coeff[k][0], coeff[k][1]
            ...
            psiJ += psiC(r, phi, m, n, zeros, b)\
                * (eigenvecs_T2[k+2*dim, nState])\
                + psiS(r, phi, m, n, zeros, b)\
                * (eigenvecs_T2[k+3*dim, nState])
...
def largeBasFuncComp(dim, coeff, eigenvecs_T2, cs, numRootsToShow=10):
    """Analysis of the components of the transformed Hamiltonian
    in terms of original basis functions"""
    ...
    for i in range(4*dim):
        ...
        coeff1[i] = sign(eigenvecs_T2[i, 0]) + ' X' + cs[i]\
...
def psi(r, phi, m, n, zeros, b):
    ...
    result = (np.sqrt(1/np.pi) / (b * sp.jv(m+1, zeros[np.abs(m), n-1]))
...
def psiRadial(r, m, n, zeros, b):  # Radial part of the basis function
    ...
    result = ((-1)**m * np.sqrt(2) / (b * sp.jv(m+1, zeros[np.abs(m), n-1]))
...
def V11(r, m1, n1, m2, n2, L, b, zeros):
    ...
    result = (psiRadial(r, m1, n1, zeros, b) * psiRadial(r, m2, n2, zeros, b)
              * r * (L**2 + 4*r**2) / 4 / 2)
...
def V_plus(r, m1, n1, m2, n2, L, b, zeros, c):
    """For test purposes: V+ potential energy matrix elements (adiabatic rep),
    much slower than using functions precompiled in c"""
...
if __name__ == "__printMatrix__":
    printMatrix()
if __name__ == "__psi__":
    psi()
if __name__ == "__psiRadialc__":
    psiRadial()
